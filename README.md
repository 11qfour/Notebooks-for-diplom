...
├── tests/ # тесты
│   ├── test_ram_usage.py #  ресурсное тестирование при работе с onnx моделью
│   │── locustfile.py	# нагрузочное тестирование 
│   └── test_api_scenarios.py     # функциональное тестирование
├── notebooks/
│   ├── clustering_tests/
│   │   │── test_samples/
...
В проекте реализованы автоматизированные функциональные и нагрузочные тесты.

### 1. Функциональное тестирование
Скрипт проверяет основные сценарии использования API: Happy Path, обработку неизвестных паттернов, SQL-инъекции, спецсимволы и валидацию длины текста.

**Запуск:**
```bash
# Убедитесь, что API запущен (localhost:8000)
python tests/test_api_scenarios.py
```

**Ожидаемый результат**: OK по всем тест-кейсам.

### 2. Нагрузочное тестирование (Locust)
Проверка устойчивости системы под нагрузкой (требование ТЗ: >5 RPS).
...
```
#### 3. Откройте интерфейс: `http://localhost:8089`

### 3. Ресурное тестирование

Подтверждение соблюдения лимитов RAM:

```bash
python -m tests.test_ram_usage
```